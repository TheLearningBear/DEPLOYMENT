...
def like(request, id):
  ...
  this_quote.liked_by=Users.objects.get(id=request.session["user_id"])
  this_quote.save()
  all_likes=Quotes.objects.liked_by.all().count()
  context={
    "id":id,
    "totallike":all_likes,
  }
...
def quotes(request,id):
  ...
  return render(request, 'user_app/quotes.html', context)
